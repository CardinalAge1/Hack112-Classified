...
def color_balance(image):
    ...
    image_array[0] = numpy.minimum(
        image_array[0] * (average_g / numpy.average(image_array[0])), 255)
    image_array[2] = numpy.minimum(
        image_array[2] * (average_g / numpy.average(image_array[2])), 255)
...
class ColorFinder(cozmo.annotate.Annotator):
    ...
    def __init__(self, robot: cozmo.robot.Robot):
        ...
        self.color_to_find_index = POSSIBLE_COLORS_TO_FIND.index(
            self.color_to_find)
        ...
        # initially set to center screen
        self.last_known_blob_center = (DOWNSIZE_WIDTH / 2, DOWNSIZE_HEIGHT / 2)
    ...
    def apply(self, image, scale):
        ...
        for i in range(DOWNSIZE_WIDTH):
            for j in range(DOWNSIZE_HEIGHT):
                ...
                cozmo.annotate.add_polygon_to_image(
                    image, points_seq, 1.0, 'green', self.pixel_matrix.at(i, j).value)

        text = cozmo.annotate.ImageText(
            'Looking for {}'.format(self.color_to_find), color='white')
        ...
        if self.state != LOOK_AROUND_STATE:
            ...
            points_seq = (pt1, pt2, pt3, pt4)
            cozmo.annotate.add_polygon_to_image(
                image, points_seq, 1.0, 'black', 'gold')
    ...
    def toggle_color_to_find(self):
        ...
        self.color_selector_cube.set_lights(
    ...
    def update_pixel_matrix(self, downsized_image):
        ...
        for i in range(self.pixel_matrix.num_cols):
            for j in range(self.pixel_matrix.num_rows):
                ...
                self.pixel_matrix.at(i, j).set(
                    self.approximate_color_of_pixel(r, g, b))
    ...
    def get_low_res_view(self):
        ...
        '''
        image = self.robot.world.latest_image.raw_image
        downsized_image = image.resize(
            (DOWNSIZE_WIDTH, DOWNSIZE_HEIGHT), resample=Image.LANCZOS)
    ...
    def on_finding_a_blob(self, blob_center, blob_size):
        ...
        amount_to_move_head = radians(
            self.fov_y.radians * (.5 - float(y) / DOWNSIZE_HEIGHT))
        amount_to_rotate = radians(
            self.fov_x.radians * (.5 - float(x) / DOWNSIZE_WIDTH))
    ...
    def turn_toward_blob(self, amount_to_move_head, amount_to_rotate):
        ...
        Args:
           ...
           amount_to_rotate (cozmo.util.Angle):
               the perceived horizontal distance of the blob from center-screen
        '''
        self.abort_actions(self.tilt_head_action,
                           self.rotate_action, self.drive_action)
        new_head_angle = self.robot.head_angle + amount_to_move_head
        self.tilt_head_action = self.robot.set_head_angle(
            new_head_angle, warn_on_clamp=False, in_parallel=True)
        self.rotate_action = self.robot.turn_in_place(
            amount_to_rotate, in_parallel=True)
        if self.state == FOUND_COLOR_STATE:
            self.amount_turned_recently += amount_to_move_head.abs_value + \
                amount_to_rotate.abs_value
    ...
    def drive_toward_color_blob(self):
        ...
        if self.should_start_new_action(self.drive_action):
            self.drive_action = self.robot.drive_straight(distance_mm(
                500), speed_mmps(300), should_play_anim=False, in_parallel=True)
    ...
    def turn_toward_last_known_blob(self):
        ...
        x, y = self.last_known_blob_center
        amount_to_move_head = radians(
            self.fov_y.radians * (.5 - y / DOWNSIZE_HEIGHT))
        amount_to_rotate = radians(
            self.fov_x.radians * (.5 - x / DOWNSIZE_WIDTH)) * 4
        self.turn_toward_blob(amount_to_move_head, amount_to_rotate)
    ...
    async def start_lookaround(self):
        ...
        if self.look_around_behavior == None or not self.look_around_behavior.is_active:
            ...
            if self.state == LOOK_AROUND_STATE:  # state may have changed due to turn_toward_last_known_blob
                self.abort_actions(self.tilt_head_action,
                                   self.rotate_action, self.drive_action)
                self.look_around_behavior = self.robot.start_behavior(
    ...
    def cubes_connected(self):
        ...
        self.color_selector_cube = self.robot.world.get_light_cube(
            cozmo.objects.LightCube1Id)
        self.grid_cube = self.robot.world.get_light_cube(
            cozmo.objects.LightCube2Id)
        self.white_balance_cube = self.robot.world.get_light_cube(
...
class BlobDetector():
    ...
    def __init__(self, matrix, color_to_find):
        ...
        # prevents a lot of irrelevant blobs from being processed
        self.filter_blobs_dict_by_size(5)
        ...
        for i in range(self.matrix.num_cols):
            for j in range(self.matrix.num_rows):
                if self.matrix.at(i, j).value == self.color_to_find:
                    ...
                    should_merge = matches_left and matches_above and self.above_and_left_blobs_are_different(
                        i, j)
                    if should_merge:
    ...
    def above_and_left_blobs_are_different(self, i, j):
        ...
        if i == 0 or j == 0:
            return False
        above_and_left_different = (self.keys.at(
            i - 1, j).value != self.keys.at(i, j - 1).value)
    ...
    def filter_blobs_dict_by_size(self, n):
        ...
        '''
        self.blobs_dict = dict((blob, list_of_points) for blob,
                               list_of_points in self.blobs_dict.items() if len(list_of_points) >= n)
    ...
    def get_largest_blob_key(self):
        ...
        if len(values) > 0:
            longest_points_list = functools.reduce(
                lambda largest, current: largest if (largest > current) else current, values)
...
class MyMatrix():
    ...
    def __init__(self, num_cols, num_rows):
        ...
        self._matrix = [[MatrixValueContainer() for _ in range(self.num_rows)]
    ...
    def surrounded(self, i, j):
        ...
        if i != 0 and i != self.num_cols - 1 and j != 0 and j != self.num_rows - 1:
            left_value, up_value, right_value, down_value = self.get_neighboring_values(
                i, j)
